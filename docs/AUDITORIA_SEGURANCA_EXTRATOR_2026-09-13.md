...
- Salvamento da conta testado no site: nome temporário salvo e confirmado na tela; nome original restaurado e confirmação exibida. Sem mudança em email, senha, CNPJ ou assinatura.
- Teste real de abrir XML detectou permissão EXECUTE ausente para service_role em dois predicados privados chamados pelos novos RPCs invoker. Corrigido na migração20260914032613, sem conceder nada a anon/authenticated. Reteste SQL SET LOCAL ROLE service_role confirmou vínculo e janela; teste de falha durante gravação atômica confirmou preservação do A1 ativo e terminou em rollback. Esse teste é mais representativo que executar apenas como postgres.
- Reteste no navegador após a correção: documento armazenado abriu, título/número corretos e geração de PDF terminou com botão habilitado. A prévia PDF embutida exibiu fallback no navegador integrado; evento de download não foi observado em45s. Portanto, abertura/retorno do documento confirmados, download final do DANFE e pacote ZIP NÃO certificados nesta rodada.
- Publicação frontend0c9b4e86 confirmada no navegador: aviso “Credencial SEFAZ pendente” aparece em Pontos de atenção; novas datas por dia, importação validada e nome editável presentes. A inspeção React orientou a proteção contra respostas de consultas antigas e a limpeza dos estados de carregamento.
- Auditoria final do Supabase sem alertas ERROR, mas ainda com WARN sobre proteção de senhas vazadas desativada, pg_net no schema public e4 RPCs definer intencionais com validação de vínculo. Isso não equivale a um pentest completo.

## Limite da entrega

Este lote corrigiu vulnerabilidades confirmadas e falhas do Extrator, foi versionado e publicado. NÃO concluiu a revisão manual de todos os166 endpoints, de todas as combinações de papéis/contas, de todas as páginas públicas ou das pontes sem fonte disponível. Ainda há11 alertas de dependências e as pendências descritas acima. Sem emissão oficial, sem cobrança e sem substituição de credenciais reais.
