...
s = s.replace('  const applyCnpjLookup = (lookup: any) => {', '  const applyCnpjLookup = (lookup: any, meta: any = {}) => {', 1)
s = s.replace("      'phone',\n      'postal_code',", "      'phone',\n      'mobile',\n      'postal_code',", 1)
old = """      if (lookup?.state_registration) {
        next.ie_indicator = lookup.ie_indicator || '1';
        next.icms_taxpayer = lookup.icms_taxpayer !== false;
      }
      return next;"""
new = """      if (lookup?.state_registration) {
        next.ie_indicator = lookup.ie_indicator || '1';
        next.icms_taxpayer = lookup.icms_taxpayer !== false;
      }
      ...
      return next;"""
assert old in s, 'apply lookup merge block not found'
s = s.replace(old, new, 1)

old_msg = """    setMessage(lookup?.state_registration
...
new_msg = """    const sourceCount = Array.isArray(meta?.sources?.federal) ? meta.sources.federal.length : 0;
...
assert old_msg in s, 'lookup message block not found'
s = s.replace(old_msg, new_msg, 1)
s = s.replace('            onResolved={data => applyCnpjLookup(data)}', '            onResolved={(data, meta) => applyCnpjLookup(data, meta)}', 1)

lookup_end = """        {!isCatalog && form.person_type === 'legal' && (
          <SaasCnpjLookup
...
"""
if '<OfficialRegistrySummary form={form}' not in s:
    assert lookup_end in s, 'CNPJ component block not found'
    s = s.replace(lookup_end, lookup_end + "\n        {!isCatalog && form.person_type === 'legal' && <OfficialRegistrySummary form={form} />}\n", 1)

marker = 'function PartyEditor({ section, form, set, hideTaxIdForLegal = false }:'
if 'function OfficialRegistrySummary' not in s:
    component = r'''function OfficialRegistrySummary({ form }: { form: any }) {
...
'''
    assert marker in s, 'PartyEditor marker not found'
    s = s.replace(marker, component + marker, 1)

s = s.replace('<Field label="Contato" value={form.contact_name} onChange={value => set(\'contact_name\', value)} />', '<Field label="Contato" value={form.contact_name} onChange={value => set(\'contact_name\', value)} hint="Contato interno/comercial; não é inferido automaticamente pelo quadro societário." />', 1)
s = s.replace('<Field label="Inscrição municipal" value={form.municipal_registration} onChange={value => set(\'municipal_registration\', value)} />', '<Field label="Inscrição municipal" value={form.municipal_registration} onChange={value => set(\'municipal_registration\', value)} hint="Não existe uma consulta nacional única; informe quando necessário." />', 1)
p.write_text(s)

p = Path('src/components/saas/SaasCnpjLookup.tsx')
s = p.read_text()
s = s.replace("    setMessage('Consultando Receita e dados estaduais...');", "    setMessage('Consultando CNPJ.ws, BrasilAPI e dados estaduais...');", 1)
old = """      setSuccess(true);
      setMessage(
...
assert old in s, 'CNPJ success message not found'
s = s.replace(old, new, 1)
p.write_text(s)
