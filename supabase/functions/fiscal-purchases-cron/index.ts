...
async function backfillIssuerEventSales(admin:any,company:any,cnpj:string,historyStart:string){
  const {data:events,error}=await admin.from("fiscal_dfe_events")
    .select("access_key,event_type,event_at,nsu,schema_name,xml")
    .eq("company_id",company.id).not("access_key","is",null).order("event_at",{ascending:true}).limit(5000);
  if(error)throw error;
  const monthFloor=String(historyStart||"").slice(0,7).replace("-","");
  const byKey=new Map<string,any>();
  for(const e of events||[]){
    const key=dg(e.access_key);
    if(key.length!==44||key.slice(6,20)!==cnpj||key.slice(20,22)!=="55")continue;
    const keyMonth="20"+key.slice(2,6);
    if(monthFloor&&keyMonth<monthFloor)continue;
  ...
  for(let i=0;i<keys.length;i+=250){
    const {data:rows,error:ee}=await admin.from("fiscal_sales_documents").select("access_key").eq("company_id",company.id).in("access_key",keys.slice(i,i+250));
    if(ee)throw ee;
    for(const row of rows||[])existing.add(dg(row.access_key));
  }
  const missing=keys.filter(k=>!existing.has(k));
  if(!missing.length)return 0;
  const now=new Date().toISOString();
  const refs=missing.map(key=>{const e=byKey.get(key),actor=dg(tag(String(e?.xml||""),"CNPJ")||tag(String(e?.xml||""),"CPF")),cancelled=String(e?.event_type||"")==="110111";return{
    company_id:company.id,uf:String(company.uf||"").toUpperCase(),model:"55",access_key:key,
    document_number:String(Number(key.slice(25,34))),series:String(Number(key.slice(22,25))),issue_date:e?.event_at||null,
    status:cancelled?"Cancelada":"Referenciada por evento oficial",total_value:null,
    recipient_document:actor&&actor!==cnpj?actor:null,recipient_name:null,xml:null,source:"national_dfe_issuer_event",
    source_reference:{event_nsu:e?.nsu||null,event_type:e?.event_type||null,event_schema:e?.schema_name||null,issue_month_from_key:"20"+key.slice(2,4)+"-"+key.slice(4,6),xml_pending:true},
    updated_at:now
  }});
  const {error:ie}=await admin.from("fiscal_sales_documents").upsert(refs,{onConflict:"company_id,access_key",ignoreDuplicates:true});
  if(ie)throw ie;
  const recs=missing.map(key=>{const e=byKey.get(key),cancelled=String(e?.event_type||"")==="110111";return{
    company_id:company.id,model:"55",series:String(Number(key.slice(22,25))),note_number:Number(key.slice(25,34)),
    status:cancelled?"cancelled":"found",access_key:key,issue_date:e?.event_at||null,cstat:cancelled?"101":"100",
    xmotivo:cancelled?"Cancelamento referenciado por evento oficial":"Chave referenciada por evento oficial do Ambiente Nacional",
    attempts:1,last_checked_at:now,resolved_at:now,updated_at:now,xml_status:"pending",xml_attempts:0,
    detail_status:"pending",detail_attempts:0,event_status:"found",event_attempts:1
  }});
  const {error:re}=await admin.from("fiscal_sales_reconciliation").upsert(recs,{onConflict:"company_id,model,series,note_number",ignoreDuplicates:true});
  if(re)throw re;
