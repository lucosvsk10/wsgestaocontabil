...
export function renderDanfseOfficialHtml(d:DanfseData){
  ...
  const previewPatch = [
    ...
    '[data-bind="serviceClassification"]{white-space:normal!important;line-height:10.56px!important;height:auto!important;overflow:visible!important}',
    '[data-bind="serviceDescription"]{white-space:pre-wrap!important;line-height:10.56px!important;height:auto!important;overflow:visible!important}',
    '</style>',
    '<script>',
    '(function(){',
    ...
    'function adjustService(){var cls=document.querySelector(\'[data-bind="serviceClassification"]\');var desc=document.querySelector(\'[data-bind="serviceDescription"]\');var page=document.querySelector(".page");if(!cls||!desc||!page)return;var pageTop=page.getBoundingClientRect().top;var clsRect=cls.getBoundingClientRect();var clsBottom=clsRect.bottom-pageTop;var labelY=Math.max(411.53,clsBottom+4);var labels=Array.from(document.querySelectorAll(".t.c0")).filter(function(el){var style=el.getAttribute("style")||"";var t=(el.textContent||"").trim();return style.indexOf(",411.53)")>=0&&(t==="Descrição"||t==="do"||t==="Serviço");});labels.forEach(function(el){setY(el,labelY);});setY(desc,labelY+8.93);}',
    'function scheduleAdjust(){requestAnimationFrame(function(){requestAnimationFrame(adjustService);});setTimeout(adjustService,60);setTimeout(adjustService,180);}',
