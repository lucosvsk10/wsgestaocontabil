...
export async function buildDanfsePdf(d:DanfseData){
  ...
  field(page,normal,bold,156.47,431.22,"Tipo de Tributação do ISSQN",d.municipalTax.type);
  field(page,normal,bold,301.04,431.22,"Município / Sigla UF / País de Incidência do ISSQN",d.municipalTax.incidence,{labelWidth:286});
  field(page,normal,bold,11.91,450.29,"BC ISSQN",d.municipalTax.base);
  field(page,normal,bold,156.47,450.29,"Alíquota Aplicada",d.municipalTax.rate);
  field(page,normal,bold,301.04,450.29,"Retenção do ISSQN",d.municipalTax.retention);
  field(page,normal,bold,445.61,450.29,"ISSQN Apurado",d.municipalTax.amount);

  field(page,normal,bold,156.47,469.87,"IRRF",d.federalTax.irrf);
  field(page,normal,bold,301.04,469.87,"Contribuição Previdenciária - Retida",d.federalTax.previdencia,{labelSize:6});
  field(page,normal,bold,445.61,469.87,"Contribuições Sociais - Retidas",d.federalTax.sociais,{labelSize:6});
  field(page,normal,bold,11.91,488.94,"PIS - Débito Apuração Própria",d.federalTax.pis);
  field(page,normal,bold,156.47,488.94,"COFINS - Débito Apuração Própria",d.federalTax.cofins);
  field(page,normal,bold,301.04,488.94,"Descrição Contrib. Sociais - Retidas",d.federalTax.retainedDescription);

  field(page,normal,bold,156.47,508.51,"CST / cClassTrib",d.ibsCbs.cstClass);
  drawTop(page,bold,"Indicador de Operação / Código IBGE Incidência / Município Incidência / Sigla UF",301.04,508.51,6,{maxWidth:286});
  drawTop(page,normal,d.ibsCbs.operationIncidence||"-",301.04,515.22,7,{maxWidth:286});
  field(page,normal,bold,11.91,527.59,"Exclusões e Reduções da Base de Cálculo",d.ibsCbs.exclusions);
  field(page,normal,bold,156.47,527.59,"Base de Cálculo Após Exclusões e Reduções",d.ibsCbs.base);
  field(page,normal,bold,301.04,527.59,"Red. Alíquota IBS / Red. Alíquota CBS",d.ibsCbs.reduction);
  field(page,normal,bold,445.61,527.59,"Alíquota - IBS UF / IBS Mun",d.ibsCbs.rateUfMun);
  field(page,normal,bold,11.91,546.66,"Alíq. Efetiva Municipal - IBS",d.ibsCbs.effectiveMunicipal);
  field(page,normal,bold,156.47,546.66,"Valor Apurado Municipal - IBS",d.ibsCbs.amountMunicipal);
  field(page,normal,bold,301.04,546.66,"Alíq. Efetiva Estadual - IBS",d.ibsCbs.effectiveState);
  field(page,normal,bold,445.61,546.66,"Valor Apurado Estadual - IBS",d.ibsCbs.amountState);
  field(page,normal,bold,11.91,565.74,"Valor Total Apurado - IBS",d.ibsCbs.totalIbs);
  field(page,normal,bold,156.47,565.74,"Alíquota - CBS",d.ibsCbs.cbsRate);
  field(page,normal,bold,301.04,565.74,"Alíquota Efetiva - CBS",d.ibsCbs.cbsEffective);
  field(page,normal,bold,445.61,565.74,"Valor Total Apurado - CBS",d.ibsCbs.cbsTotal);

  field(page,normal,bold,156.47,585.31,"VALOR DA OPERAÇÃO / SERVIÇO",d.totals.operation);
  field(page,normal,bold,301.04,585.31,"Desconto Incondicionado",d.totals.unconditionalDiscount);
  field(page,normal,bold,445.61,585.31,"Desconto Condicionado",d.totals.conditionalDiscount);
  field(page,normal,bold,11.91,604.39,"Total das Retenções (ISSQN / Federais)",d.totals.retentions);
  field(page,normal,bold,156.47,604.39,"VALOR LÍQUIDO DA NFS-e",d.totals.net);
  field(page,normal,bold,301.04,604.39,"Total do IBS/CBS",d.totals.ibsCbs);
  field(page,normal,bold,445.61,604.39,"VALOR LÍQUIDO DA NFS-e + IBS/CBS",d.totals.netPlusIbsCbs);

  drawTop(page,normal,"Inf. Cont.: "+(d.additionalInfo||"-"),11.91,636.06,7,{maxWidth:565});
  drawTop(page,normal,d.approximateTaxes||"-",11.91,643.98,7,{maxWidth:565});
