...
Deno.serve(async req=>{
  ...
  try{
    ...
    const tpAmb=environment==="production"?"1":"2",im=digits(p.municipal_registration),dados:any={tpAmb,dhEmi:new Date(),verAplic:"WS-SAAS-2.1",serie:String(raw.serie),nDPS:String(raw.numero),dCompet:new Date(),tpEmit:"1",cLocEmi:mun,prest:{CNPJ:cnpj,...(im?{IM:im}:{}),regTrib:reg},serv:{locPrest:{cLocPrestacao:munPrest},cServ:{cTribNac:code,...(digits(raw.codigoTributacaoMunicipal)?{cTribMun:digits(raw.codigoTributacaoMunicipal)}:{}),xDescServ:String(raw.descricao).trim(),cNBS:nbs}},valores:{vServPrest:{vServ:Number(raw.valor)},trib:{tribMun:{tribISSQN:String(raw.tributacaoIss||"1"),tpRetISSQN:raw.issRetido?"2":"1"},totTrib:{pTotTribFed:0,pTotTribEst:0,pTotTribMun:0}}}};
