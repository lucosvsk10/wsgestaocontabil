...
Deno.serve(async req0=>{try{
 ...
 for(const [status,emission] of variants){const q=new URLSearchParams({ufEntrada:"AL",numeroCnpjEntrada:issuer,numeroCacealEntrada:ie,ufDestino:"",numeroCnpjDestino:"",numeroCacealDestino:"",dataEmissaoInicial:brDate(start)+" 00:00",dataEmissaoFinal:brDate(end)+" 23:59",codigoStatus:status,codigoTipoEmissao:emission});const url=app+"arquivozip/notasFiscais.zip?"+q.toString();const r=await req(url,{headers:{accept:"application/zip,application/octet-stream,*/*",referer:app}},cookie);cookie=cookieMerge(cookie,r.headers);const ok=zipSig(r.buf);attempts.push({http:r.status,type:r.headers.get("content-type")||"",bytes:r.buf.length,zip:ok,status_filter:status,emission_filter:emission});if(r.status>=200&&r.status<300&&ok){zip=r.buf;chosen={status,emission};break}}
