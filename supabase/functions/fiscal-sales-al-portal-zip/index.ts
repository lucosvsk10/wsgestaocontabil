...
Deno.serve(async req0=>{try{
 ...
 if(!zip&&(storageBucket||storagePath)){
  if(!["saas-private","xml-nfe"].includes(storageBucket)||!/^imports\/[a-z0-9/_-]+\.zip$/i.test(storagePath))return J({error:"invalid_storage_zip_path",storage_bucket:storageBucket,storage_path:storagePath},400);
 ...
 if(!zip){
  ...
  const app="https://nfeas.sefaz.al.gov.br/";let cookie=session.cookie;landing=await req(app,{headers:{accept:"text/html,*/*"}},cookie);cookie=cookieMerge(cookie,landing.headers);
  landingText=new TextDecoder().decode(landing.buf);
  diagnosticLinks=[...landingText.matchAll(/(?:href|src|action)=["']([^"'#]+)["']/gi)].map(m=>m[1]).filter(v=>/nfe|relat|gwt|consulta|app|menu/i.test(v)).slice(0,80);
  diagnosticFrames=[...landingText.matchAll(/<iframe[^>]+src=["']([^"']+)["']/gi)].map(m=>m[1]).slice(0,30);
  diagnosticScripts=[...landingText.matchAll(/<script[^>]+src=["']([^"']+)["']/gi)].map(m=>m[1]).slice(0,50);
  inlineRoutes=[...new Set([...landingText.matchAll(/["']([^"']*(?:nfe|relat|gwt|consulta)[^"']*)["']/gi)].map(m=>m[1]).filter(v=>v.length<260))].slice(0,80);
  rpcCount=await gwtCount(cookie,{cnpj:issuer,ie,start,end});const rpcText=String(rpcCount?.response||"");const permissionDenied=/não possui permissão|nao possui permissao/i.test(rpcText);const permissionGranted=Boolean(rpcCount?.http===200&&rpcCount?.ok&&!permissionDenied&&/^\/\/OK/.test(rpcText));if(action==="permission")return J({ok:true,company_id:cid,period:{start,end},permission:permissionGranted?"granted":permissionDenied?"denied":"unknown",rpc_count:rpcCount});
  const variants=[["","-1"],["A,C,D","-1"],["100,101,302","-1"],["A","-1"]];
 ...
 if(action==="sync"){
   ...
   for(let i=0;i<unique.length;i+=100){
     const [{error:salesError},{error:dfeError},{error:reconciliationError}]=await Promise.all([
       a.from("fiscal_sales_documents").upsert(salesRows.slice(i,i+100),{onConflict:"company_id,access_key"}),
       a.from("fiscal_dfe_documents").upsert(dfeRows.slice(i,i+100),{onConflict:"user_id,cnpj,environment,uf_code,nsu"}),
       a.from("fiscal_sales_reconciliation").upsert(reconciliationRows.slice(i,i+100),{onConflict:"company_id,model,series,note_number"})
     ]);
     if(salesError)throw salesError;if(dfeError)throw dfeError;if(reconciliationError)throw reconciliationError;saved+=Math.min(100,unique.length-i);
   }
   const nfce=unique.filter(d=>d.model==="65"),seriesCounts=new Map<string,number>();for(const d of nfce)seriesCounts.set(d.series,(seriesCounts.get(d.series)||0)+1);const primarySeries=[...seriesCounts.entries()].sort((x,y)=>y[1]-x[1])[0]?.[0];
   if(primarySeries){const stream=nfce.filter(d=>d.series===primarySeries),numbers=stream.map(d=>Number(d.number)).filter(Number.isFinite),floor=Math.min(...numbers),latest=Math.max(...numbers),total=latest-floor+1,pending=Math.max(0,total-stream.length);const{error:stateError}=await a.from("fiscal_sales_sync_state").upsert({company_id:cid,status:"queued",latest_number:latest,cursor_number:latest,initial_floor_number:floor,history_start_month:start.slice(2,4)+start.slice(5,7),found_documents:stream.length,reconciliation_total:total,reconciliation_found:stream.length,reconciliation_resolved:stream.length,reconciliation_pending:pending,reconciliation_complete:pending===0,reconciliation_started_at:now,reconciliation_completed_at:pending===0?now:null,xml_expected:stream.length,xml_saved:stream.length,xml_pending:0,xml_failed:0,xml_complete:true,last_error:null,next_scheduled_at:now,updated_at:now},{onConflict:"company_id"});if(stateError)throw stateError;}
   const counts=new Map<string,number>();for(const d of unique)counts.set(d.model,(counts.get(d.model)||0)+1);for(const [model,count] of counts){const documentType=model==="65"?"sale_nfce65":"sale_nfe55";await a.from("fiscal_source_reconciliation").upsert({company_id:cid,document_type:documentType,period_start:start,period_end:end,status:"ok",source_name:"SEFAZ/AL portal - Entradas e Saídas",source_confirmed:true,source_count:count,site_count:count,missing_count:0,extra_count:0,reason:"XML oficial enumerado pelo portal estadual.",checked_at:now,updated_at:now},{onConflict:"company_id,document_type,period_start,period_end"}).catch(()=>{});}
 }
 return J({ok:true,version:"storage-v2",company_id:cid,period:{start,end},rpc_count:rpcCount,zip_bytes:zip.length,entries:entries.length,xml_entries:entries.filter(n=>/\.xml$/i.test(n)).length,documents:unique.length,by_model:unique.reduce((o:any,d:any)=>(o[d.model]=(o[d.model]||0)+1,o),{}),saved,chosen,attempts,preview:unique.slice(0,20).map(d=>({key:d.key,model:d.model,series:d.series,number:d.number,issue:d.issue,status:d.status}))});
