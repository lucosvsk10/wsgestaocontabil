...
Deno.serve(async req0=>{try{
 ...
 const issuer=dg(c.cnpj),ie=dg(c.inscricao_estadual);const rpcCount=await gwtCount(cookie,{cnpj:issuer,ie,start,end});const action=String(b.action||"probe");const rpcText=String(rpcCount?.response||"");const permissionDenied=/não possui permissão|nao possui permissao/i.test(rpcText);const permissionGranted=Boolean(rpcCount?.http===200&&rpcCount?.ok&&!permissionDenied&&/^\/\/OK/.test(rpcText));if(action==="permission")return J({ok:true,company_id:cid,period:{start,end},permission:permissionGranted?"granted":permissionDenied?"denied":"unknown",rpc_count:rpcCount});const variants=[["","-1"],["A,C,D","-1"],["100,101,302","-1"],["A","-1"]];const attempts:any[]=[];let zip:Uint8Array|null=null,chosen:any=null;
