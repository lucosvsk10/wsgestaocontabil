...
Deno.serve(async req=>{
 ...
 try{
  ...
  const black=rgb(0.07,0.07,0.07),gray=rgb(.94,.94,.94),white=rgb(1,1,1);
  ...
  if(cancelled)page.drawText("CANCELADA",{x:138,y:y-52,size:46,font:bold,color:rgb(.82,.12,.12),opacity:.16,rotate:degrees(15)});
  ...
  if(!isNFSe){
    ...
    const cols=[22,205,50,38,48,48,48,50,42],heads=["#","Produto / Serviço","NCM","CFOP","Qtd.","Unit.","Total","BC ICMS","ICMS"];
