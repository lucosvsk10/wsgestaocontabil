...
async function buildNfce(doc: any, xml: string) {
  ...
  const separator = (before = 4.5, after = 7.5) => {
    y -= before;
    dashedLine();
    y -= after;
  };
  const centeredSection = (title: string, size = 7.2, b = false) => {
    separator(4.5, 6.5);
    draw(title, size, b, 'center');
    y -= 1.5;
    separator(0, 7.5);
  };
  const paymentHeader = () => {
  ...
  centeredSection('DETALHE DA VENDA', 7.2);
...
, num(tag(tot,'vNF') || doc.value), true, 8.7);
