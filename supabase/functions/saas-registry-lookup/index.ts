...
function mergeFederal(ws: any | null, brasil: any | null, cnpj: string) {
  ...
  merged.icms_taxpayer = Boolean(
    ws?.state_registration ||
    brasil?.state_registration ||
    ws?.icms_taxpayer ||
    brasil?.icms_taxpayer
  );
...
async function enrichStateRegistry(admin: any, cnpj: string, base: any) {
  ...
  data.registry = {
    ...
    state_registry_status:
      stateData?.state_registry_status || data.registry?.state_registry_status || '',
...
Deno.serve(async req => {
  ...
  try {
    ...
    const denied = limited(await consume(admin, 'registry_lookup', user.id, 30, 600));
    ...
    const platformAdmin = roles?.some((row: any) =>
      ['admin', 'fiscal', 'contabil', 'geral'].includes(row.role)
    );
