...
Deno.serve(async req=>{try{
 ...
 for(const c of companies||[]){
  ...
  if(targetKey){const {data,error}=await admin.from("fiscal_dfe_documents").select(cols).eq("company_id",c.id).eq("model","55").eq("access_key",targetKey).eq("full_xml",false).limit(1);if(error)throw error;addRows(data||[])}
  else{
