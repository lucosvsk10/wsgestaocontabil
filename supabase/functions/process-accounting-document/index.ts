...
Plano da empresa: ${JSON.stringify(body.chart_of_accounts)}.`;
 ...
 const response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({model:Deno.env.get("OPENAI_ACCOUNTING_MODEL")||"gpt-5.5",instructions:prompt,input:[{role:"user",content:[{type:"input_text",text:"Leia os documentos e gere os lançamentos estruturados da folha."},...fileInputs]}],text:{format:{type:"json_schema",name:"payroll_entries",strict:true,schema}}})});
