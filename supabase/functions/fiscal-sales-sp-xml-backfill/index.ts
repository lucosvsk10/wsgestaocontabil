...
Deno.serve(async req=>{try{
  ...
  for(const row of rows||[]){const key=dg(row.access_key),now=new Date().toISOString();try{
    ...
    const raw=String(o.text||""),docs:any[]=[];const rx=/<docZip\b([^>]*)>([\s\S]*?)<\/docZip>/gi;let m;
    ...
    const full=docs.find(d=>d.full);if(!full){if(docs.length){summaryOnly++;failures.push({key,error:"summary_only",schemas:docs.map(d=>d.schema)});continue}throw Error("no_document:"+String(tag(raw,"cStat")||"")+":"+String(tag(raw,"xMotivo")||""))}
