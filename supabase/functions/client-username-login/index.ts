...
const STANDARD_FIRST_ACCESS_PASSWORD = "5BWasgc1@";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (attempt?.locked_until && new Date(attempt.locked_until) > now) return json({ error: "Muitas tentativas. Aguarde alguns minutos e tente novamente." }, 429);
    ...
    if (portalUser.must_change_password && password === STANDARD_FIRST_ACCESS_PASSWORD) {
      const { error: resetError } = await admin.auth.admin.updateUserById(portalUser.id, { password: STANDARD_FIRST_ACCESS_PASSWORD });
    ...
    return json({ access_token: signInData.session.access_token, refresh_token: signInData.session.refresh_token, expires_in: signInData.session.expires_in, user: { id: signInData.user.id, username: String(portalUser.username || '').toUpperCase(), must_change_password: Boolean(portalUser.must_change_password) } });
  } catch (error) {
