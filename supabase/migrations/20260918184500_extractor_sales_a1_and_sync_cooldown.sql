-- Extrator: sales verification is driven by the active A1 + SVRS/Vercel flow.
-- Legacy state-portal credentials remain available for diagnostics, but no longer block sales sync.
-- Manual correction is limited to one request per company every five minutes.
...
begin
  ...
  v_rate_key :=
    auth.uid()::text || '|' ||
    v_account.id::text || '|' ||
    coalesce(_company_id::text, 'all');

  select * into v_rate
  from public.consume_rate_limit(
    'extractor_manual_sync_company',
    v_rate_key,
    1,
    300
  );
...
-- Existing companies imported while the legacy gate was active should be retried.
...
set
  ...
  next_scheduled_at = now(),
  updated_at = now()
where s.status = 'waiting_state_credentials'
  and exists (
    select 1
    from public.extractor_companies ec
    where ec.fiscal_company_id = s.company_id
      and ec.status = 'active'
  )
  and exists (
    select 1
    from public.fiscal_certificates cert
    where cert.company_id = s.company_id
      and cert.is_active = true
      and cert.valid_until >= current_date
  );
