-- Keep document list payload lean; the full XML is fetched only when a user opens a document.
CREATE OR REPLACE FUNCTION public.extractor_company_documents(_company_id uuid, _start date DEFAULT (CURRENT_DATE - 30), _end date DEFAULT CURRENT_DATE)
 RETURNS jsonb
 LANGUAGE plpgsql
