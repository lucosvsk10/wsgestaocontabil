...
type Plan = { product: Product; label: string; price: number; description: string; details: string[] };
...
const planCatalog: Record<PlanCode, Plan> = {
  issuer_monthly: {
    ...
    details: ['NF-e, NFC-e, NFS-e, CT-e e MDF-e', 'Cadastros, produtos e histórico em um só lugar', 'Suporte direto da equipe WS'],
  },
  extractor_commercial: {
    ...
    details: ['Compras e vendas organizadas por CNPJ', 'Até 20.000 XML por mês', 'Download dos arquivos para a rotina contábil'],
  },
  extractor_enterprise: {
    ...
    details: ['Empresas e XML sem limite', 'Compras e vendas organizadas por CNPJ', 'Download dos arquivos para a rotina contábil'],
  },
};
...
const productContent = {
  issuer: {
    ...
    description: 'Cadastre a forma de pagamento e use o Emissor por 7 dias. Você só será cobrado depois do período gratuito.',
  ...
  extractor: {
    ...
    description: 'Escolha o volume da sua carteira e teste por 7 dias. Compras e vendas ficam disponíveis e organizadas por empresa.',
...
export default function BillingOnboardingPage({ product }: { product: Product }) {
  ...
  const initialPlan = requestedPlan && planCatalog[requestedPlan]?.product === product ? requestedPlan : fallbackPlan;
  ...
  const startCheckout = async () => {
    ...
    try {
      ...
      if (invokeError || responseMessage) throw new Error(responseMessage || 'checkout_unavailable');
    ...
    <main className={`billing-page billing-${product}`}>
      ...
      <section className="billing-shell">
        <div className="billing-copy">
          ...
          <section className="billing-step" aria-labelledby="billing-plan-title">
            ...
            <div className="billing-step-body">
              <h2 id="billing-plan-title">{product === 'issuer' ? 'Seu acesso' : 'Escolha o plano'}</h2>
              <p>{product === 'issuer' ? 'Um único plano, sem limite de emissões.' : 'Você pode mudar de plano depois, conforme sua carteira crescer.'}</p>
              <div className={`billing-plan-list ${product === 'issuer' ? 'is-single' : ''}`}>
                {productPlans.map(([code, plan]) => (
                  <button type="button" key={code} aria-pressed={planCode === code} className={planCode === code ? 'active' : ''} onClick={() => setPlanCode(code)}>
                    <span><b>{plan.label}</b><small>{plan.description}</small></span>
                    <strong>R$ {plan.price}<small>/mês</small></strong>
          ...
          <section className="billing-step" aria-labelledby="billing-mode-title">
            ...
            <div className="billing-step-body">
              ...
              <div className="billing-mode">
                <button type="button" aria-pressed={billingMode === 'recurring'} className={billingMode === 'recurring' ? 'active' : ''} onClick={() => setBillingMode('recurring')}>
                  <span className="billing-choice-icon"><RefreshCw /></span>
                  <span><b>Assinatura mensal</b><small>7 dias grátis, depois R$ {selected.price}/mês</small></span>
                  <i aria-hidden="true" />
                </button>
                <button type="button" aria-pressed={billingMode === 'one_time'} className={billingMode === 'one_time' ? 'active' : ''} onClick={() => setBillingMode('one_time')}>
                  <span className="billing-choice-icon"><CreditCard /></span>
                  <span><b>Acesso por 30 dias</b><small>R$ {selected.price} agora, sem renovação automática</small></span>
                  <i aria-hidden="true" />
                </button>
              </div>
            </div>
        ...
        <aside className="billing-summary">
          <header><span>Resumo</span><h2>{selected.label}</h2></header>
          <ul>{selected.details.map(item => <li key={item}><Check /> <span>{item}</span></li>)}</ul>
          <dl>
            <div><dt>{billingMode === 'recurring' ? 'Primeiros 7 dias' : 'Período contratado'}</dt><dd>{billingMode === 'recurring' ? 'Grátis' : '30 dias'}</dd></div>
            <div><dt>{billingMode === 'recurring' ? 'Depois do teste' : 'Renovação'}</dt><dd>{billingMode === 'recurring' ? `R$ ${selected.price}/mês` : 'Não automática'}</dd></div>
            <div className="billing-total"><dt>Total hoje</dt><dd>R$ {totalToday}</dd></div>
          </dl>
          {billingMode === 'recurring' ? <p className="billing-trial-note">Se este produto já teve um teste grátis nesta conta, a primeira mensalidade será cobrada ao confirmar.</p> : null}
          <label className="billing-terms">
            <input type="checkbox" checked={accepted} onChange={event => setAccepted(event.target.checked)} />
            <span>Li e aceito os <Link to="/termos-de-servico" target="_blank">Termos de Serviço</Link> e a <Link to="/politica-de-privacidade" target="_blank">Política de Privacidade</Link>.</span>
          </label>
          {error ? <div className="billing-error" role="alert"><strong>Não foi possível continuar</strong><span>{error}</span></div> : null}
          <button type="button" className="billing-submit" disabled={!accepted || loading} onClick={() => void startCheckout()}>
            {loading ? 'Abrindo o Mercado Pago...' : billingMode === 'recurring' ? 'Ativar 7 dias grátis' : 'Comprar 30 dias'}
            {loading ? null : <ArrowRight />}
          </button>
          <small className="billing-provider"><LockKeyhole /> Seus dados de pagamento ficam no Mercado Pago</small>
        </aside>
      </section>
    </main>
  );
...
function CheckoutReturn({ status }: { status: string }) {
  ...
  return <div className={`billing-return is-${status}`} role="status">{message}</div>;
}
...
function friendlyCheckoutError(message: string) {
  ...
  if (normalized.includes('mercado pago ainda não foi configurado')) return 'O pagamento está temporariamente indisponível. Fale com a equipe WS.';
