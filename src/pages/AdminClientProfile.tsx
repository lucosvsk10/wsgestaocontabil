...
export default function AdminClientProfile(){
 ...
 const updateAccessUsername=async()=>{if(!companyId||!access||accessBusy||!confirmPassword)return;setAccessBusy(true);setError('');setNotice('');try{await confirmAccessPassword(confirmPassword);const result=await callAccess({action:'update_username',company_id:companyId,username:accessUsername,confirm_password:confirmPassword});setConfirmAction(null);setConfirmPassword('');setAccessEditing(false);setNotice(`Usuário atualizado para @${result.username}. O login anterior deixou de funcionar.`);await refreshCompanies();await load()}catch(e){setError(e instanceof Error?e.message:String(e))}finally{setAccessBusy(false)}};
 const resetAccessPassword=async()=>{if(!companyId||!access||accessBusy||!confirmPassword)return;setAccessBusy(true);setError('');setNotice('');try{await confirmAccessPassword(confirmPassword);await callAccess({action:'reset_password',company_id:companyId,confirm_password:confirmPassword});setConfirmAction(null);setConfirmPassword('');setNotice('Senha redefinida para a senha padrão da WS. A senha anterior deixou de funcionar e a troca será obrigatória no próximo acesso.');await refreshCompanies();await load()}catch(e){setError(e instanceof Error?e.message:String(e))}finally{setAccessBusy(false)}};
