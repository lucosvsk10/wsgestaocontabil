...
import { getCurrentProductAccess, getPendingCheckoutDestination } from '@/utils/auth/productAccess';
...
const ClientLogin = () => {
  ...
  const resolveDestination = async (userId: string, metadata?: Record<string, unknown>) => {
    const redirectPath = new URLSearchParams(location.search).get('redirect');
    if (
      redirectPath?.startsWith('/') &&
      !redirectPath.startsWith('//') &&
      !redirectPath.includes('\\') &&
      !redirectPath.startsWith('/client')
    )
      return redirectPath;
    ...
    const pendingCheckout = await getPendingCheckoutDestination();
    if (pendingCheckout) return pendingCheckout;
    const intendedProduct = metadata?.signup_product;
    const intendedPlan = typeof metadata?.signup_plan === 'string' ? metadata.signup_plan : '';
    if (intendedProduct === 'extractor')
      return `/assinar/extrator?plan=${encodeURIComponent(intendedPlan || 'extractor_commercial')}`;
    if (intendedProduct === 'issuer') return '/assinar/emissor?plan=issuer_monthly';
    return '/home-preview';
  };

  const signInWithUsername = async (username: string, currentPassword: string) => {
    const response = await fetch(
      'https://nadtoitgkukzbghtbohm.supabase.co/functions/v1/client-username-login',
      {
        method: 'POST',
        headers: {
    ...
    if (sessionError || !data.user)
      throw sessionError || new Error('Não foi possível iniciar a sessão.');
    return data.user;
  };
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      navigate(await resolveDestination(user.id, user.user_metadata), { replace: true });
    } catch (caughtError: unknown) {
      const message =
        caughtError instanceof Error && caughtError.message.includes('Muitas tentativas')
          ? caughtError.message
      ...
      <div className="ws-login-shell">
        <section className="ws-login-presentation">
          ...
          <Link
            ...
            aria-label="Ir para o site da WS Gestão Contábil"
          >
          ...
          <div className="ws-login-presentation-copy">
            ...
            <p>
              Emita documentos, organize empresas e acompanhe sua rotina fiscal com segurança e
              clareza.
            </p>
        ...
        <section className="ws-login-access">
          <div className="ws-login-theme-switch" role="group" aria-label="Aparência da tela">
            <button type="button" aria-pressed={!light} onClick={() => setTheme('default')}>
              Padrão
            </button>
            <button type="button" aria-pressed={light} onClick={() => setTheme('light')}>
              Claro
            </button>
          </div>
          <div className="ws-login-form-wrap">
            <Link
              to="/home-preview"
              className="ws-login-mobile-brand"
              aria-label="Ir para o site da WS Gestão Contábil"
            >
              <img src={logo} alt="WS Gestão Contábil" />
            </Link>
            <header className="ws-login-heading">
              ...
              <p>
                Clientes do escritório entram com nome de usuário. Contas dos produtos fiscais
                também podem usar e-mail.
              </p>
            </header>
            <form onSubmit={handleSubmit} className="ws-login-form">
              ...
              <input
                ...
                autoCapitalize="none"
                spellCheck={false}
                placeholder="seu.usuario ou email@gmail.com"
                value={credential}
            ...
            <div
              style={{
                marginTop: 18,
                paddingTop: 18,
                borderTop: '1px solid rgba(127,145,165,.22)',
                textAlign: 'center',
                fontSize: 13,
              }}
            >
