...
export default function AdminFeature() {
  ...
  const lock = () => { setToken(""); setCertificateBase64(""); setCertificatePassword(DEFAULT_A1_PASSWORD); setCertificateName(""); setCertificate(null); setResult(null); setError(""); };
...
function SaleForm({mode,data,setData}:{mode:Mode;data:typeof saleInitial;setData:React.Dispatch<React.SetStateAction<typeof saleInitial>>}) {
  ...
  return <>
    ...
    <div className="grid min-w-0 gap-4 md:grid-cols-2">
      ...
      <Field label="CRT"><select className="h-10 min-w-0 w-full rounded-md border bg-background px-3 text-sm" value={data.crt} onChange={e=>u('crt',e.target.value)}><option value="1">1 · Simples Nacional</option><option value="3">3 · Regime normal</option></select></Field>
