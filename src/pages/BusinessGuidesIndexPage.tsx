...
const BusinessGuidesIndexPage = () => {
  ...
  return <div className="public-page guides-index-page" style={{background:'#020b1b'}}>
    ...
    <main className="guides-index-main">
      ...
      <section className="public-guide-grid" aria-label="Guias para empresas">{guides.map((guide, index) => <Link className="public-guide-card" to={`/guias/${guide.slug}`} key={guide.slug}><img className="public-guide-thumb" src={guide.heroImage} alt={guide.heroImageAlt} loading={index === 0 ? 'eager' : 'lazy'} /><span className="public-guide-number">0{index + 1}</span><small>{guide.eyebrow} · <Clock3 aria-hidden="true" /> {guide.readingTime}</small><h2>{guide.title}</h2><p>{guide.description}</p><strong>Ler guia <ArrowRight /></strong></Link>)}</section>
