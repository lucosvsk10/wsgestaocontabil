...
export default function SaasApp() {
  ...
  let content: any;
  if (active === "Início" && organization?.id && !setupComplete && !setupDismissed) content = <SaasSetupGuide organizationId={organization.id} organizationName={organization?.name} profile={profile} certificateConfigured={certificateConfigured} emissionsCount={emissions.length} onOpenCompany={() => setActive("Minha Empresa")} onStartEmission={() => {setSelectedDocument(null);setActive("Emissão")}} onDismiss={dismissSetup}/>;
  else if (active === "Início") content = <SaasDashboard organizationName={organization?.name} emissions={emissions} onNew={() => { setSelectedDocument(null); setActive("Emissão"); }} onReports={() => setActive("Relatórios")} />;
  ...
  return <div className="saas-admin-light min-h-screen bg-background text-foreground" style={lightVars}>
    ...
    <aside className="fixed bottom-0 left-0 top-20 z-50 flex w-72 flex-col border-r border-border bg-white">
      <div className="border-b border-border px-5 py-5"><button onClick={() => setActive("Minha Empresa")} className="flex aspect-square w-full items-center justify-center overflow-hidden rounded-2xl border border-border bg-[#f7f8fa] p-6 transition hover:bg-[#f1f3f6]">{logoUrl ? <img src={logoUrl} alt="Logo da empresa" className="max-h-full max-w-full object-contain" /> : <span className="text-center text-xs font-medium text-muted-foreground">Adicionar logomarca</span>}</button></div>
    ...
    <main className="min-h-screen bg-[#f3f5f7] pl-72 pt-20"><div className="mx-auto w-full max-w-[1540px] px-4 py-5 sm:px-5 sm:py-6 lg:px-8">{content}</div></main>
  </div>;
}
