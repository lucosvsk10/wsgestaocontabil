...
const stepsByDocument: Record<string, string[]> = {
 ...
 'NFS-e': ['Pessoas', 'Serviço', 'Valores', 'Emitir NFS-e'],
...
function EmissionForm({
  ...
  useEffect(() => {
    ...
    setMsg('');
    setStepAlert('');
    const savedStep = documentType === 'NFS-e' && savedDraft?.tab === 'Revisão' ? 'Emitir NFS-e' : documentType === 'NFS-e' && savedDraft?.tab === 'Impostos' ? 'Valores' : savedDraft?.tab;
...
<FiscalRecordPicker label="Cliente" value={form.customerId} items={partyRecords}
  ...
  if (documentType === 'NFS-e')
    content = <NfseOfficialFlow step={step} form={form} setField={set} setForm={setForm} profile={profile} customers={customers} services={services} customer={customer} service={service} onOpenCadastro={onOpenCadastro} />;
  if (documentType === 'CT-e')
    content =
      step === 0 ? (
