...
export default function SaasIssuedNotes({emissions,onNew}:Props){
 ...
 const rows=useMemo(()=>scoped.filter((item:any)=>{
   ...
   const matchesTab=tab==="Todas"||
    (tab==="Autorizadas"&&item.status==="authorized")||
    (tab==="Rascunhos"&&item.status==="draft")||
    (tab==="Em andamento"&&item.status==="validated")||
    (tab==="Canceladas"&&item.status==="cancelled")||typeLabel(item)===tab;
   return matchesQuery&&matchesTab;
 }),[scoped,query,tab]);
 const pages=Math.max(1,Math.ceil(rows.length/pageSize)),safePage=Math.min(page,pages),visibleRows=rows.slice((safePage-1)*pageSize,safePage*pageSize);
 ...
 return <div className="saas-issued-notes w-full space-y-4 text-[#111827]">
  ...
  <section className="overflow-hidden rounded-md border border-[#d7dde5] bg-white shadow-sm">
   ...
   <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left"><thead><tr className="border-b border-[#d8dee6] bg-[#eef1f4]"><th className="px-4 py-3">Emissão</th><th className="px-4 py-3">Arquivo</th><th className="px-4 py-3">Nota / chave</th><th className="px-4 py-3">Destinatário / emitente</th><th className="px-4 py-3">Operação</th><th className="px-4 py-3 text-right">Valor</th><th className="px-4 py-3 text-right">Ações</th></tr></thead>
    <tbody>{visibleRows.map((item:any,index:number)=>{const d=new Date(item.external_issue_date||item.created_at||0);return <tr key={item.id||index} className={`border-b border-[#e1e6ec] ${index%2===0?"bg-white":"bg-[#f6f7f8]"} hover:bg-[#eef2f5]`}>
      ...
      <td className="px-4 py-3.5"><div className="flex items-center gap-2"><span className="font-semibold">{item.number||"—"}</span><span className="text-[10px] text-[#7b8794]">/ {item.series||"—"}</span><span className="rounded bg-[#e7f2f7] px-2 py-1 text-[9px] font-semibold text-[#27708f]">{typeLabel(item)}</span><Status value={String(item.status||"draft")}/></div><div className="mt-1 max-w-[230px] truncate text-[9px] text-[#98a2b3]">{item.access_key||"Chave não informada"}</div></td>
      <td className="px-4 py-3.5"><div className="max-w-[240px] truncate font-medium text-[#344054]">{item.recipient_name||"Sem destinatário"}</div><div className="mt-0.5 text-[10px] text-[#7b8794]">{item.recipient_tax_id||"Documento não informado"}</div></td>
      <td className="px-4 py-3.5"><div className="max-w-[220px] truncate text-[#344054]">{itemName(item)}</div><div className="mt-0.5 text-[10px] text-[#7b8794]">{item.source==="imported"?"Documento extraído":"Emitida pelo SaaS"}</div></td>
      <td className="px-4 py-3.5 text-right font-semibold">{money(Number(item.total||0))}</td>
      <td className="px-4 py-3.5"><div className="flex justify-end gap-2"><button onClick={()=>setSelected(item)} className="inline-flex items-center gap-1 rounded border border-transparent bg-transparent px-2 py-1 text-xs font-semibold text-[#344054]"><FileText className="h-3.5 w-3.5"/>Visualizar</button>{item.xml&&<button onClick={()=>downloadXml(item)} className="rounded border border-[#d4dce5] bg-white px-2 py-1 text-[10px] font-semibold text-[#344054]">XML</button>}</div></td>
