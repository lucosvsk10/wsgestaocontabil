...
export default function FiscalCodeField({kind,label,value,onChange,required=false,onResolved}:{kind:Kind;label:string;value:string;onChange:(code:string)=>void;required?:boolean;onResolved?:(record:Code)=>void}){
 ...
 return <div className="fe-code-field">
  ...
  <div className="fe-code-input"><Input id={id} inputMode="numeric" value={value||''} maxLength={kind==='ncm'?10:8}
    placeholder={kind==='ncm'?'8 dígitos':'6 dígitos'} onFocus={()=>{if(!rows.length&&!loading&&!failed)void fetchCodes()}} onChange={event=>{const code=event.target.value.replace(/\D/g,'').slice(0,kind==='ncm'?8:6);lastResolved.current='';onChange(code)}}/>
   <Button type="button" variant="ghost" aria-label={`Consultar ${label}`} onClick={show}><Search size={16}/><span>Consultar</span></Button></div>
  <small className="fe-field-hint">{exact?.description|| (kind==='ncm'?'Pesquise na tabela NCM pelo código ou descrição. Não é o código interno / SKU.':'Pesquise a classificação nacional pelo código ou nome do serviço. A descrição oficial será aproveitada no cadastro.')}</small>
  <Dialog open={open} onOpenChange={setOpen}><DialogContent className="ws-record-dialog" onOpenAutoFocus={event=>{event.preventDefault();input.current?.focus()}}>
   <header><p className="ws-eyebrow">CLASSIFICAÇÃO FISCAL</p><DialogTitle>{kind==='ncm'?'Encontrar um NCM':'Código de Tributação Nacional'}</DialogTitle><DialogDescription>Escolha o código que corresponde à operação. No cadastro de serviços, a descrição oficial será preenchida automaticamente.</DialogDescription></header>
   ...
   <div className="ws-record-results">{!loading&&!failed&&matches.slice(0,limit).map(row=><button type="button" className="ws-record-result" key={row.id} onClick={()=>{choose(row.id);setOpen(false)}}><span className="fe-code-number">{row.code}</span><span className="ws-record-body"><strong>{row.name}</strong></span><ArrowRight size={16}/></button>)}
