...
const auxTitle=(documentType:string)=>documentType==="CT-e"?"DACTE":documentType==="MDF-e"?"DAMDFE":documentType==="NFC-e"?"DANFE NFC-e":"DANFE";
 ...
 *{box-sizing:border-box}
 ...
 w.document.open();
 w.document.write(`<!doctype html><html><head><title>${title}</title><meta charset="utf-8"/>${inheritedStyles}${printCss}</head><body>${node.outerHTML}</body></html>`);
 w.document.close();
 let printed=false;
 const doPrint=()=>{if(printed)return;printed=true;w.focus();w.print()};
 if(w.document.readyState==="complete")setTimeout(doPrint,500);else w.addEventListener("load",()=>setTimeout(doPrint,500),{once:true});
