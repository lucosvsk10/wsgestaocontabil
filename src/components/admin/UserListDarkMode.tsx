  ...
  const UserTable = ({ usersList, title, isAdmin = false }: { usersList: any[], title: string, isAdmin?: boolean }) => (
    <div className="mb-12">
      <div className="flex items-center gap-3 mb-6">
        {isAdmin ? (
          <UserCheck className="h-6 w-6 text-[#efc349]" />
        ) : (
          <Users className="h-6 w-6 text-[#efc349]" />
        )}
        <h2 className="text-2xl font-semibold text-[#efc349] uppercase tracking-wide">
          {title}
        </h2>
        ...
        <Table>
          ...
          <TableBody>
            {filterAndSortUsers(usersList).map((user, index) => (
              <TableRow 
                key={user.id} 
                className={`border-b border-[#efc349]/10 hover:bg-[#efc349]/5 transition-colors ${
                  index % 2 === 1 ? 'bg-white/[0.015]' : ''
                }`}
              >
                {!isAdmin && (
                  <TableCell className="text-[#f4f4f4] font-medium">
                    {getUserName(user)}
                  </TableCell>
                )}
                ...
                {!isAdmin && (
                  <TableCell>
                    <div className="flex gap-2">
                      <Button
                        size="sm"
                        className="bg-[#1e293b] hover:bg-[#efc349] hover:text-[#020817] text-white border-none transition-all"
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        className="bg-[#374151] hover:bg-[#4b5563] text-white border-[#374151]"
                        onClick={() => {
                      ...
                      <Button
                        size="sm"
                        className="bg-[#7f1d1d] hover:bg-[#dc2626] text-white border-none transition-all"
            ...
            {filterAndSortUsers(usersList).length === 0 && (
              <TableRow>
                <TableCell 
                  colSpan={isAdmin ? 2 : 4} 
                  className="text-center py-8 text-[#b3b3b3]"
                >
  ...
  return (
    <div className="space-y-8 p-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-[#efc349] mb-2">
        ...
        <Button 
          ...
          className="bg-[#efc349] hover:bg-[#d4a73a] text-[#020817] font-semibold transition-all"
        >
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8">
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="pl-10 bg-[#0b0f1c] border-[#efc349]/30 text-[#f4f4f4] placeholder-[#b3b3b3] focus:border-[#efc349]"
          />
        </div>
