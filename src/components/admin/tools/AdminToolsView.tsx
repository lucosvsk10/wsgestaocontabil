...
export const AdminToolsView: React.FC = () => {
  ...
  return <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {tools.map((tool, index) => <div key={index} className="p-8 text-center space-y-6 transition-all duration-300 hover:scale-105 group bg-white dark:bg-transparent rounded-xl border border-gray-100 dark:border-none hover:shadow-lg dark:hover:shadow-none">
            ...
            <Button variant="outline" className="w-full transition-all duration-300 hover:scale-105" onClick={tool.action}>
              Acessar
            </Button>
          </div>)}
      </div>
