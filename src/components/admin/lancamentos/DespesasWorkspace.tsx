...
interface Props { company: string; month: string; year: string; onFileCountChange?: (count: number) => void; onStatusChange?: (status: WorkspaceStatus) => void; }
...
export function DespesasWorkspace({ company, month, year, onFileCountChange, onStatusChange }: Props) {
  ...
  const canExport = grouped.length > 0 && !issues.length && !outside && !missing && !mixed && detailedTotal === groupedTotal;
  ...
  const confirm = async () => { if (!pending) return; const chart = new Map(accounts.map(account => [account.reducedCode, account.description])); const resolved = pending.entries.map(row => ({ ...row, debitDescription: chart.get(row.debitCode) || row.debitDescription, creditDescription: chart.get(row.creditCode) || row.creditDescription })); setEntries(current => [...current, ...resolved.filter(row => !current.some(saved => saved.id === row.id))]); setIssues(current => [...current, ...pending.issues]); setIgnoredRows(current => current + pending.ignoredRows); setFiles(current => Array.from(new Set([...current, ...pendingFiles.map(file => file.name)]))); await saveWorkspaceFiles(scope, pendingFiles); setPending(null); setPendingFiles([]); onStatusChange?.("review"); };
  ...
  return <section className="mt-5 space-y-5">
    <div className="rounded-md border border-border bg-background p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><h3 className="font-semibold">Despesas de {competence}</h3><Button variant="outline" disabled={reading} onClick={() => fileInput.current?.click()}>{reading ? "Lendo arquivo..." : `Importar despesas de ${competence}`}</Button><input ref={fileInput} type="file" multiple accept=".xlsx,.xls,.csv" className="sr-only" onChange={readFiles}/></div><div className="mt-4 border-t border-border pt-4 text-sm text-muted-foreground">{files.length ? <div className="flex flex-wrap gap-4">{files.map(file => <span key={file} className="text-foreground">{file}</span>)}<button className="underline" onClick={clear}>Limpar importação</button></div> : "Nenhum arquivo selecionado nesta competência."}</div></div>
    {pending && <div className="rounded-md border border-border bg-background"><div className="flex items-center justify-between border-b border-border p-4"><div><h3 className="font-semibold">Prévia da importação</h3><p className="text-xs text-muted-foreground">Cinco primeiras linhas</p></div><div className="flex gap-2"><Button variant="outline" onClick={() => setPending(null)}>Cancelar</Button><Button onClick={confirm} disabled={!pending.entries.length}>Confirmar importação</Button></div></div><LedgerTable rows={pending.entries.slice(0,5)}/></div>}
    <Tabs defaultValue="transcricao"><TabsList className="grid h-auto w-full grid-cols-3 gap-2 bg-transparent p-0"><WorkflowTab value="transcricao" label="Transcrição"/><WorkflowTab value="lancamentos" label="Lançamentos"/><WorkflowTab value="conferencia" label="Conferência"/></TabsList>
      <TabsContent value="transcricao" className="mt-5"><div className="rounded-md border border-border bg-background"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-border p-4"><Tabs value={view} onValueChange={setView}><TabsList><TabsTrigger value="detalhada">Visão detalhada</TabsTrigger><TabsTrigger value="agrupada">Visão agrupada</TabsTrigger><TabsTrigger value="leitura">Conferência da leitura</TabsTrigger></TabsList></Tabs>{view === "agrupada" && <div className="flex items-center gap-2 text-xs"><span className="text-muted-foreground">Agrupar por</span>{(["debit","credit"] as ExpenseGroupSide[]).map(side => <button key={side} onClick={() => setGroupSide(side)} className={cn("rounded border px-3 py-1.5", groupSide === side && "bg-foreground text-background")}>{side === "debit" ? "Débito" : "Crédito"}</button>)}</div>}</div>
        {view === "detalhada" && <LedgerTable rows={entries} editable update={update}/>} {view === "agrupada" && <LedgerTable rows={displayGrouped}/>} {view === "leitura" && <div className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-3"><Stat label="Lançamentos válidos" value={entries.length}/><Stat label="Linhas com problema" value={issues.length}/><Stat label="Linhas informativas" value={ignoredRows}/><Stat label="Fora da competência" value={outside}/><Stat label="Contrapartidas múltiplas" value={mixed}/><Stat label="Total identificado" value={currency(detailedTotal)}/>{issues.map(issue => <p key={issue.id} className="col-span-full text-sm text-muted-foreground">{issue.fileName} · linha {issue.row}: {issue.message}</p>)}</div>}
      ...
      <TabsContent value="conferencia" className="mt-5"><div className="flex flex-col gap-5 rounded-md border border-border bg-background p-5 sm:flex-row sm:items-center sm:justify-between"><div className="grid flex-1 gap-4 sm:grid-cols-4"><Stat label="Detalhado" value={currency(detailedTotal)}/><Stat label="Agrupado" value={currency(groupedTotal)}/><Stat label="Diferença" value={currency(groupedTotal-detailedTotal)}/><Stat label="Pendências" value={issues.length+outside+missing+mixed}/></div><Button disabled={!canExport} onClick={() => onStatusChange?.("done")}>Marcar despesas como OK</Button></div></TabsContent>
    </Tabs>
  </section>;
}
function WorkflowTab({ value, label }: { value: string; label: string }) { return <TabsTrigger value={value} className="min-h-14 border border-border data-[state=active]:bg-foreground data-[state=active]:text-background">{label}</TabsTrigger>; }
