...
const inputCellClass = "h-8 rounded-none border-0 bg-transparent px-2 shadow-none focus:border-foreground/30 focus:ring-foreground/10 focus-visible:ring-1 focus-visible:ring-foreground/30 dark:bg-transparent dark:focus:border-white/30 dark:focus:ring-white/10";
function contextKey(companyId: string) { return `ws:lancamentos:last-context:${companyId}`; }
...
export function LancamentosWorkspace() {
  ...
  const activeModuleLabel = useMemo(() => modules.find((module) => module.key === selectedModule)?.label ?? "Folha", [selectedModule]);

  const applyModuleFiles = (module: ModuleKey, selectedFiles: File[], target?: { month: string; year: string }) => {
    setFilesByModule((current) => ({ ...current, [module]: [...current[module], ...selectedFiles] })); setSelectedModule(module); setActiveTab("transcricao");
  ...
  useEffect(() => {
    ...
    Promise.all([loadWorkspaceData<{ transcriptionRows: TranscriptionRow[]; launchRows: LaunchRow[] }>(genericDataKey), loadWorkspaceFiles(genericScope)]).then(([saved, storedFiles]) => { setTranscriptionRows(saved?.transcriptionRows ?? []); setLaunchRows(saved?.launchRows ?? []); setFilesByModule(current => ({ ...current, [selectedModule]: storedFiles })); setGenericLoaded(true); });
  ...
  return <div className="mx-auto w-full max-w-[1720px] px-4 pb-12 pt-5 sm:px-6 lg:px-8">
    <header className="flex flex-col gap-4 border-b border-border pb-5 xl:flex-row xl:items-end xl:justify-between"><div className="min-w-0"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">Operação contábil</p><h1 className="mt-1 text-2xl font-semibold tracking-tight text-foreground sm:text-3xl">Lançamentos</h1><p className="mt-1 truncate text-sm text-muted-foreground">{company.name}<span className="px-2 text-border">/</span>{selectedMonthLabel} de {year}<span className="px-2 text-border">/</span>Não iniciado</p></div><div className="flex w-full flex-col gap-3 sm:flex-row xl:w-auto"><CompanySelector company={company} companies={companies} onSelect={selectCompany} /></div></header>
    <div className="grid min-h-[720px] lg:grid-cols-[220px_minmax(0,1fr)]">
      ...
      <main className="min-w-0 py-5 lg:pl-6">
        ...
        <nav className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Módulos contábeis">{modules.map((module) => { const status = (yearStatuses[selectedMonth] ?? emptyStatuses())[module.key]; const statusLabel = status === "done" ? "Concluído" : status === "review" ? "Aguardando conferência" : "Aguardando importação"; return <button key={module.key} type="button" onClick={() => setSelectedModule(module.key)} className={cn("grid min-h-24 grid-cols-[minmax(0,1fr)_30%] overflow-hidden rounded-md bg-background text-left transition-colors", selectedModule === module.key ? "bg-muted/80" : "hover:bg-muted/45")}><span className="flex flex-col justify-center px-4 py-3"><span className="text-sm font-medium text-foreground">{module.label}</span><span className="mt-1 text-xs text-muted-foreground">{persistedModuleCounts[module.key] || filesByModule[module.key].length ? `${persistedModuleCounts[module.key] || filesByModule[module.key].length} arquivo(s)` : statusLabel}</span></span><span className={cn("flex items-center justify-center px-2 text-center text-[10px] font-semibold uppercase tracking-wide", status === "done" ? "bg-emerald-500/20 text-emerald-700 dark:text-emerald-300" : status === "review" ? "bg-amber-400/20 text-amber-700 dark:text-amber-300" : "bg-muted text-muted-foreground")}>{statusLabel}</span></button>; })}</nav>
        {selectedModule === "despesas" ? <DespesasWorkspace key={`${company.id}-${year}-${selectedMonth}`} company={company.id} month={selectedMonth} year={year} onFileCountChange={handleExpenseFileCount} onStatusChange={(status) => setModuleStatus("despesas", status)} onCompetenceChange={(nextMonth, nextYear) => { setYear(nextYear); setSelectedMonth(nextMonth); }} /> : selectedModule === "folha" ? <FolhaWorkspace key={`${company.id}-${year}-${selectedMonth}`} company={company.id} month={selectedMonth} year={year} onStatusChange={(status) => setModuleStatus("folha", status)} onCompetenceChange={(nextMonth,nextYear)=>{setYear(nextYear);setSelectedMonth(nextMonth)}} /> : <>
          <section className="mt-8 rounded-md border border-border bg-background p-6" aria-label="Importação de documentos"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><h3 className="text-base font-semibold text-foreground">{activeModuleLabel} de {selectedMonthLabel} de {year}</h3><label className="cursor-pointer rounded-md border border-border px-4 py-2.5 text-sm font-medium text-foreground transition-colors hover:bg-muted">Importar {activeModuleLabel.toLowerCase()} de {selectedMonthLabel} de {year}<input type="file" multiple accept={modules.find((module) => module.key === selectedModule)?.acceptedFiles} className="sr-only" onChange={(event) => void handleModuleFiles(selectedModule, event)} /></label></div><div className="mt-4 border-t border-border pt-4 text-sm text-muted-foreground">{filesByModule[selectedModule].length ? <div className="flex flex-wrap gap-4">{filesByModule[selectedModule].map((file, index) => <span key={`${file.name}-${index}`} className="text-foreground">{file.name}</span>)}</div> : "Nenhum arquivo selecionado nesta competência."}</div></section>
          <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-8"><TabsList className="grid h-auto w-full grid-cols-3 gap-2 bg-transparent p-0 shadow-none dark:bg-transparent">{[["transcricao", "Transcrição", transcriptionRows.length], ["lancamentos", "Lançamentos", launchRows.length], ["conferencia", "Conferência", 0]].map(([value, label, count]) => <TabsTrigger key={String(value)} value={String(value)} className="min-h-14 rounded-md border border-border bg-transparent px-4 text-muted-foreground shadow-none data-[state=active]:bg-foreground data-[state=active]:text-background data-[state=active]:shadow-none">{label}<span className="ml-2 rounded-full bg-muted px-2 py-0.5 text-[10px] tabular-nums text-muted-foreground">{count}</span></TabsTrigger>)}</TabsList>
            <TabsContent value="transcricao" className="mt-5"><div className="mb-4 flex flex-wrap items-end justify-between gap-3"><div><h3 className="text-base font-semibold text-foreground">{activeModuleLabel} · Transcrição</h3><p className="mt-1 text-sm text-muted-foreground">Documento original e dados extraídos no mesmo espaço.</p></div><Badge variant="outline" className="rounded-sm border-border font-normal text-muted-foreground dark:border-white/15">Edição local ativa</Badge></div><section className="min-w-0 rounded-md border border-border bg-background"><div className="border-b border-border px-4 py-3"><h3 className="text-sm font-semibold text-foreground">Transcrição editável</h3><p className="mt-1 text-xs text-muted-foreground">Clique em qualquer célula para corrigir.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[720px] border-collapse text-sm"><thead className="bg-muted/50 text-left text-xs text-muted-foreground"><tr><th className="w-24 border-b border-r border-border px-3 py-2 font-medium">Código</th><th className="border-b border-r border-border px-3 py-2 font-medium">Descrição extraída</th><th className="w-32 border-b border-r border-border px-3 py-2 text-right font-medium">Valor</th><th className="w-48 border-b border-border px-3 py-2 font-medium">Classificação</th></tr></thead><tbody>{transcriptionRows.map((row) => <tr key={row.id} className="border-b border-border last:border-b-0"><td className="border-r border-border"><Input className={inputCellClass} value={row.code} onChange={(event) => updateTranscription(row.id, "code", event.target.value)} /></td><td className="border-r border-border"><Input className={inputCellClass} value={row.description} onChange={(event) => updateTranscription(row.id, "description", event.target.value)} /></td><td className="border-r border-border"><Input className={cn(inputCellClass, "text-right tabular-nums")} value={row.value} onChange={(event) => updateTranscription(row.id, "value", event.target.value)} /></td><td><Input className={inputCellClass} value={row.classification} onChange={(event) => updateTranscription(row.id, "classification", event.target.value)} /></td></tr>)}{!transcriptionRows.length && <tr><td colSpan={4} className="h-32 px-4 text-center text-sm text-muted-foreground">Nenhum dado transcrito. Importe um documento ou adicione uma linha manualmente.</td></tr>}</tbody></table></div><div className="flex flex-wrap items-center justify-between gap-3 border-t border-border px-4 py-3"><div className="flex flex-wrap gap-4 text-xs text-muted-foreground"><span>{transcriptionRows.length} linhas</span><span>Totais serão calculados após a transcrição</span></div><Button className="border-border bg-transparent text-foreground shadow-none hover:bg-muted dark:border-white/15 dark:hover:bg-white/5" variant="outline" size="sm" onClick={addTranscriptionRow}>Adicionar linha</Button></div></section></TabsContent>
            <TabsContent value="lancamentos" className="mt-5"><div className="mb-4 flex flex-wrap items-end justify-between gap-3"><div><h3 className="text-base font-semibold text-foreground">Planilha de lançamentos</h3><p className="mt-1 text-sm text-muted-foreground">Mesmo formato do arquivo final para o Calima.</p></div><Badge variant="outline" className="rounded-sm border-border font-normal text-muted-foreground dark:border-white/15">{launchRows.length} lançamentos</Badge></div><section className="overflow-hidden rounded-md border border-border bg-background"><div className="overflow-x-auto"><table className="w-full min-w-[1280px] border-collapse text-sm"><thead className="bg-muted/50 text-left text-xs text-muted-foreground"><tr>{[["Data", "w-32"], ["Histórico variável", "min-w-[310px]"], ["Débito", "w-24"], ["Descrição débito", "min-w-[200px]"], ["C.C. débito", "w-28"], ["Crédito", "w-24"], ["Descrição crédito", "min-w-[200px]"], ["C.C. crédito", "w-28"], ["Valor", "w-36 text-right"]].map(([label, width]) => <th key={label} className={cn("border-b border-r border-border px-3 py-2 font-medium last:border-r-0", width)}>{label}</th>)}</tr></thead><tbody>{launchRows.map((row) => <tr key={row.id} className="border-b border-border last:border-b-0">{([["date", row.date], ["history", row.history], ["debit", row.debit], ["debitDescription", row.debitDescription], ["debitCostCenter", row.debitCostCenter], ["credit", row.credit], ["creditDescription", row.creditDescription], ["creditCostCenter", row.creditCostCenter], ["value", row.value]] as [keyof LaunchRow, string][]).map(([field, value]) => <td key={field} className="border-r border-border last:border-r-0"><Input className={cn(inputCellClass, field === "value" && "text-right tabular-nums")} value={value} onChange={(event) => updateLaunch(row.id, field, event.target.value)} /></td>)}</tr>)}{!launchRows.length && <tr><td colSpan={9} className="h-32 px-4 text-center text-sm text-muted-foreground">Nenhum lançamento gerado. Revise a transcrição ou adicione um lançamento manualmente.</td></tr>}</tbody></table></div><div className="flex flex-wrap items-center justify-between gap-3 border-t border-border px-4 py-3"><div className="flex flex-wrap gap-4 text-xs text-muted-foreground"><span>Débitos, créditos e diferenças serão calculados após a geração</span></div><Button className="border-border bg-transparent text-foreground shadow-none hover:bg-muted dark:border-white/15 dark:hover:bg-white/5" variant="outline" size="sm" onClick={addLaunchRow}>Adicionar lançamento</Button></div></section></TabsContent>
            <TabsContent value="conferencia" className="mt-5"><div className="mb-4"><h3 className="text-base font-semibold text-foreground">Conferência da competência</h3><p className="mt-1 text-sm text-muted-foreground">Somente divergências e decisões que exigem atenção.</p></div><section className="overflow-hidden rounded-md border border-border bg-background"><div className="grid grid-cols-[minmax(120px,0.4fr)_minmax(220px,1fr)_minmax(160px,0.6fr)] bg-muted/50 px-4 py-2 text-xs font-medium text-muted-foreground"><span>Origem</span><span>Conferência</span><span>Resultado</span></div><div className="grid min-h-32 place-items-center border-t border-border px-4 text-center text-sm text-muted-foreground">A conferência será exibida quando houver documentos processados e lançamentos gerados.</div><div className="flex justify-end border-t border-border p-4"><Button onClick={() => setModuleStatus(selectedModule, "done")}>Marcar como OK</Button></div></section></TabsContent>
          </Tabs>
        </>}
      </main>
    </div>
