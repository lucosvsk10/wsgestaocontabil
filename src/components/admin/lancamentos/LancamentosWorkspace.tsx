...
export function LancamentosWorkspace() {
  ...
  useEffect(() => {
    ...
    Promise.all([loadWorkspaceData<{ transcriptionRows: TranscriptionRow[]; launchRows: LaunchRow[] }>(genericDataKey), loadWorkspaceFiles(genericScope)]).then(([saved, storedFiles]) => {
      setTranscriptionRows(saved?.transcriptionRows ?? []); setLaunchRows(saved?.launchRows ?? []); setFilesByModule(current => ({ ...current, [selectedModule]: storedFiles })); setGenericLoaded(true);
    });
        ...
        {selectedModule === "despesas" ? <DespesasWorkspace key={`${company.id}-${year}-${selectedMonth}`} company={company.id} month={selectedMonth} year={year} onFileCountChange={handleExpenseFileCount} onStatusChange={status => setModuleStatus("despesas", status)} onCompetenceChange={(nextMonth, nextYear) => { setYear(nextYear); setSelectedMonth(nextMonth); }} /> : selectedModule === "folha" ? <FolhaWorkspace key={`${company.id}-${year}-${selectedMonth}`} company={company.id} month={selectedMonth} year={year} onStatusChange={status => setModuleStatus("folha", status)} onCompetenceChange={(nextMonth,nextYear)=>{setYear(nextYear);setSelectedMonth(nextMonth)}} /> : <>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-8">
            <AccountingWorkflowSteps steps={[
              { value: "transcricao", label: "Transcrição", count: transcriptionRows.length },
              { value: "lancamentos", label: "Lançamentos", count: launchRows.length },
              { value: "conferencia", label: "Conferência", count: 0 },
            ]} />
