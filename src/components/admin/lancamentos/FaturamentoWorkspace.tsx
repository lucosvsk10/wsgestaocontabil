...
export function FaturamentoWorkspace({ company, month, year, onStatusChange, onCompetenceChange }: Props) {
  ...
  const canApprove = referenceVerified && !processing && !learning && missing === 0 && blockingDifferences.length === 0 && warnings.length === 0 && validationIssues.length === 0;
  ...
  const hydrate = (saved: SavedRevenue | RevenueProcessingResult) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      else { setReference(null); setEntries([]); setWarnings([]); setValidationIssues([]); setProcessingMeta(null); setReferenceVerified(false); }
  ...
  useEffect(() => {
    if (!loaded) return;
    void saveWorkspaceData(key, { reference, entries, comparisons, warnings, validationIssues, processingMeta, referenceVerified });
  }, [comparisons, entries, key, loaded, processingMeta, reference, referenceVerified, validationIssues, warnings]);
  ...
  const importFiles = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const detectedYear = detectRevenueYear(selected[0]);
    const targetYear = detectedYear ?? year;
    const targetScope = `${company}:${targetYear}:${month}:faturamento`;
    await saveWorkspaceFiles(targetScope, selected);
    if (targetYear !== year) {
      onCompetenceChange(month, targetYear);
      await process(selected, "import", month, targetYear);
      return;
    }
    const all = uniqueFiles([...files, ...selected]);
    setFiles(all); onStatusChange("review");
    await process(all, "import");
  };
  ...
  const updateEntry = (id: string, field: keyof RevenueEntry, value: string) => setEntries(rows => rows.map(row => {
    ...
    if (field === "debitCode" || field === "creditCode") {
      ...
      const changed = field === "debitCode" ? { ...row, debitCode: value, debitDescription: account?.description ?? "" } : { ...row, creditCode: value, creditDescription: account?.description ?? "" };
    ...
    if (field === "debitCostCenter" || field === "creditCostCenter") return { ...row, [field]: value, mappingSource: "manual" as const, mappingNeedsApproval: true, mappingConfidence: 1, mappingReason: "Centro de custo ajustado manualmente e aguardando confirmação." };
  ...
  const exportCalima = () => exportAccountingWorkbook({
    ...
    entries: entries.map(row => ({ date: row.date, amountInCents: row.amountInCents, debitCode: row.debitCode, creditCode: row.creditCode, history: row.history, debitCostCenter: row.debitCostCenter, creditCostCenter: row.creditCostCenter, debitDescription: row.debitDescription, creditDescription: row.creditDescription, referenceCode: row.rubricCode, referenceDescription: row.rubricDescription, type: row.kind, section: row.section, mappingSource: row.mappingSource, mappingReason: row.mappingReason })),
    comparisons,
  ...
  return <section className="mt-8 space-y-8">
    <div className="rounded-md border border-border bg-background p-6"><div className="flex items-center justify-between gap-5"><div><h3 className="font-semibold">Faturamento de {competence}</h3><p className="mt-1 text-xs text-muted-foreground">NFS + NF-e + DAS/PGDAS → até 3 lançamentos mensais.</p></div><div className="flex gap-2">{files.length > 0 && <Button variant="outline" disabled={processing} onClick={() => void process(files, "reprocess")}>{processing ? "Processando..." : "Reprocessar com IA"}</Button>}<Button variant="outline" disabled={processing} onClick={() => input.current?.click()}>Importar faturamento de {competence}</Button></div><input ref={input} type="file" multiple accept=".pdf" className="sr-only" onChange={event => void importFiles(event)} /></div><div className="mt-5 border-t border-border pt-5 text-sm text-muted-foreground">{files.length ? files.map(file => <span key={`${file.name}-${file.size}`} className="mr-5 text-foreground">{file.name}</span>) : "Nenhum relatório importado."}</div></div>

    <Tabs value={activeTab} onValueChange={setActiveTab}>
      <AccountingWorkflowSteps steps={[{ value: "transcricao", label: "Transcrição", count: reference ? 4 : 0 }, { value: "lancamentos", label: "Lançamentos", count: entries.length }, { value: "conferencia", label: "Conferência", count: conferenceCount }]} />

      <TabsContent value="transcricao" className="mt-7"><Header title="Faturamento · Transcrição" subtitle="Valores literais da linha da competência no relatório de faturamento." />{reference ? <RevenueReferenceTable reference={reference} title={`Transcrição de faturamento · ${competence}`} /> : <Empty text="Importe o relatório de faturamento para iniciar." />}</TabsContent>

      <TabsContent value="lancamentos" className="mt-7"><Header title="Faturamento · Lançamentos" subtitle="No máximo três lançamentos no último dia do mês." /><div className="rounded-md border border-border bg-background"><div className="flex items-center justify-between border-b border-border p-5"><span className="text-sm text-muted-foreground">{entries.length} lançamento(s) · {money(entries.reduce((sum,row) => sum + row.amountInCents, 0))}</span><Button disabled={!canExport || !entries.length} onClick={exportCalima}>Exportar para o Calima</Button></div><RevenueLedger rows={entries} update={updateEntry} title={`Lançamentos de faturamento · ${competence}`} />{!canExport && entries.length > 0 && <p className="px-5 pb-4 text-xs text-muted-foreground">{mappingsToApprove ? "Confirme o mapeamento na Conferência para liberar a exportação." : "A exportação será liberada quando os valores e mapeamentos estiverem conferidos."}</p>}</div></TabsContent>

      <TabsContent value="conferencia" className="mt-7"><Header title="Faturamento · Conferência" subtitle="NFS, NF-e, total faturado e PGDAS comparados diretamente com os lançamentos." /><div className="space-y-6 rounded-md border border-border bg-background p-6"><div className="grid gap-5 sm:grid-cols-5"><Stat label="Lançamentos" value={entries.length} /><Stat label="Diferenças" value={blockingDifferences.length} /><Stat label="Sem mapeamento" value={missing} /><Stat label="Aguardando aprovação" value={mappingsToApprove} /><Stat label="Conhecimento reutilizado" value={learnedMappings} /></div>{!referenceVerified && <div className="flex gap-2 rounded-md border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive"><AlertTriangle className="mt-0.5 h-4 w-4" />A leitura independente do relatório ainda não fechou.</div>}<RevenueComparisonTable rows={comparisons} referenceVerified={referenceVerified} />{(warnings.length > 0 || validationIssues.length > 0) && <div className="rounded-md bg-muted/50 p-4"><p className="text-sm font-medium">Pontos que exigem decisão</p>{[...new Set([...warnings, ...validationIssues])].map(issue => <p key={issue} className="mt-2 flex gap-2 text-sm text-muted-foreground"><AlertTriangle className="mt-0.5 h-4 w-4" />{issue}</p>)}</div>}{processingMeta && <p className="text-xs text-muted-foreground">Fluxo: {processingMeta.routing || processingMeta.primaryModel}</p>}<div className="flex justify-end"><Button disabled={!canApprove || !entries.length} onClick={() => void approve()}>{learning ? "Salvando conhecimento..." : mappingsToApprove ? "Confirmar e aprender" : "Marcar faturamento como OK"}</Button></div></div></TabsContent>
    </Tabs>
  </section>;
}
...
function detectRevenueYear(file: File) { return file.name.match(/faturamento[^0-9]*(20\d{2})/i)?.[1] ?? null; }
