    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" className="h-14 min-w-[360px] max-w-[560px] justify-between gap-4 rounded-xl border border-border/60 bg-card px-4 shadow-sm hover:bg-muted/30">
          <span className="flex min-w-0 items-center gap-3"><CompanyAvatar logo={selectedCompany?.logo_url} name={title} /><span className="min-w-0 text-left"><span className="block truncate text-sm font-semibold leading-5">{loading ? 'Carregando empresas...' : title}</span>{selectedCompany && <span className="block truncate text-[10px] leading-4 text-muted-foreground">{formatCnpj(selectedCompany.cnpj)}</span>}</span></span>
      ...
      <DropdownMenuContent align="center" sideOffset={8} className="z-[240] w-[440px] max-w-[calc(100vw-2rem)] rounded-xl border border-border bg-popover shadow-2xl">
