...
interface SidebarItemProps { icon: LucideIcon; label: string; active: boolean; to: string; onClick?: () => void; }
const SidebarItem: React.FC<SidebarItemProps> = ({ icon: Icon, label, active, to, onClick }) => <Link to={to} className={`group flex items-center gap-3 rounded-md border-l-2 px-4 py-2.5 transition-colors duration-200 ${active ? "border-foreground/60 bg-muted text-foreground" : "border-transparent text-muted-foreground hover:bg-muted/60 hover:text-foreground"}`} onClick={onClick}><Icon size={17} strokeWidth={1.75} /><span className="text-sm font-medium tracking-tight">{label}</span></Link>;
...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ open, onClose }) => {
  ...
  return <aside data-sidebar="true" className={`${isMobile ? 'fixed' : 'sticky top-0 h-screen self-start shrink-0'} inset-y-0 left-0 z-50 flex w-72 flex-col border-r border-border/60 bg-background text-foreground transition-all duration-300 ease-in-out ${isMobile ? open ? 'translate-x-0 shadow-2xl' : '-translate-x-full' : open ? 'translate-x-0' : '-translate-x-0 md:translate-x-0 md:w-20'}`}>
    ...
    <div className="flex h-20 shrink-0 items-center justify-center border-b border-border/60 px-5">
      <Link to="/" className="flex items-center justify-center transition-opacity hover:opacity-85">{(open || isMobile) ? <img src={theme === 'light' ? "/lovable-uploads/f7fdf0cf-f16c-4df7-a92c-964aadea9539.png" : "/lovable-uploads/fecb5c37-c321-44e3-89ca-58de7e59e59d.png"} alt="WS Gestão Contábil" className="h-7" /> : <img src={theme === 'light' ? "/lovable-uploads/83322e23-9ed8-4622-8631-8022a1d10c19.png" : "/lovable-uploads/ed055b1a-ba3e-4890-b78d-1d83e85b592b.png"} alt="WS Gestão Contábil" className="h-8" />}</Link>
    </div>
    <nav className={`flex-1 overflow-y-auto px-3 py-4 ${open || isMobile ? "space-y-5" : "space-y-3"}`}>
      {sidebarSections.map((section, sectionIndex) => <section key={section.title} className={!open && !isMobile && sectionIndex > 0 ? "border-t border-border/60 pt-3" : undefined}>
        ...
        <div className="space-y-1">{section.items.map(item => { const isLaunches = item.to === "/admin/lancamentos"; const keepOpen = currentPath.startsWith("/admin/lancamentos"); return <div key={item.label} className={isLaunches ? "group/lancamentos" : undefined}>
          {(open || isMobile) ? <SidebarItem icon={item.icon} label={item.label} active={item.active} to={item.to} onClick={isMobile ? onClose : undefined} /> : <div className={`flex justify-center rounded-lg p-3 transition-colors ${item.active ? "bg-muted text-foreground" : "hover:bg-muted/60"}`} title={item.label}><Link to={item.to} className={item.active ? "text-foreground" : "text-muted-foreground hover:text-foreground"}><item.icon size={19} /></Link></div>}
