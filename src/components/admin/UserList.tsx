        ...
        <Table>
          <TableHeader>
            <TableRow className="border-b border-gray-100 dark:border-[#efc349]/20 hover:bg-transparent">
              {!isAdmin && <TableHead className="text-[#020817] dark:text-[#efc349] font-extralight uppercase tracking-wider">
          ...
          <TableBody>
            {filterAndSortUsers(usersList).map((user, index) => <TableRow key={user.id} className={`border-b border-gray-100 dark:border-[#efc349]/10 hover:bg-gray-50 dark:hover:bg-[#efc349]/5 transition-colors ${index % 2 === 1 ? 'bg-gray-50/50 dark:bg-white/[0.015]' : ''}`}>
                {!isAdmin && <TableCell className="text-[#020817] dark:text-[#f4f4f4] font-extralight">
                    {getUserName(user)}
                  </TableCell>}
                ...
                {!isAdmin && <TableCell>
                    <div className="flex gap-2 flex-wrap">
                      <Button size="sm" variant="outline" className="bg-transparent border border-[#efc349] text-[#020817] dark:text-[#efc349] hover:bg-[#efc349]/10 font-extralight" onClick={() => navigate(`/admin/user-documents/${user.id}`)}>
                        <FileText className="h-4 w-4 mr-1" />
                        Docs
                      </Button>
                      
                      <Button size="sm" variant="outline" className="bg-transparent border border-[#efc349] text-[#020817] dark:text-[#efc349] hover:bg-[#efc349]/10 font-extralight" onClick={() => navigate(`/admin/company-data/${user.id}`)}>
                        <Building2 className="h-4 w-4 mr-1" />
                        Empresa
                      </Button>
                      <Button size="sm" variant="outline" className="bg-transparent border border-red-400 text-red-600 dark:text-red-400 hover:bg-red-400/10 font-extralight" onClick={() => handleDeleteUser(user.id)}>
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          <div className="relative flex-1">
            ...
            <Input placeholder="Buscar por nome ou email..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 bg-white dark:bg-[#0b0f1c] border-gray-200 dark:border-[#efc349]/30 font-extralight" />
          </div>
