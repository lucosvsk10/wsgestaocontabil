...
type Props = { token: string; certificateBase64?: string; certificatePassword?: string; fiscalProfile?: any; expectedCnpj?: string };
...
export function CteIssuerPanel({ token, certificateBase64 = "", certificatePassword = "", fiscalProfile, expectedCnpj = "" }: Props) {
  ...
  useEffect(() => {
    ...
    const address = fiscalProfile.endereco || {};
    const uf = String(fiscalProfile.uf || "AL").toUpperCase();
    setForm(current => ({
      ...current,
      cMunIni: fiscalProfile.codigo_municipio || current.cMunIni,
      xMunIni: fiscalProfile.municipio || current.xMunIni,
      UFIni: uf,
      emitIE: fiscalProfile.inscricao_estadual || "",
      emitNome: fiscalProfile.razao_social || fiscalProfile.nome_fantasia || "",
      emitLgr: address.logradouro || address.street || "",
      emitNro: address.numero || address.number || "",
      emitBairro: address.bairro || address.district || "",
      emitMun: fiscalProfile.codigo_municipio || "",
      emitXMun: fiscalProfile.municipio || "",
      emitCEP: address.cep || address.postal_code || "",
      emitUF: uf,
      emitCRT: fiscalProfile.regime_tributario === "simples_nacional" || !fiscalProfile.regime_tributario ? "1" : "3",
    }));
    setResult(null);
    setError("");
  }, [fiscalProfile?.id]);
  ...
  const run = async (kind: "status" | "preview" | "issue") => {
    ...
    try {
      const data = await invoke({ action: kind, engine_token: token, environment: "homologacao", expected_cnpj: expectedCnpj, ...(certificateBase64 && certificatePassword ? { certificate_base64: certificateBase64, certificate_password: certificatePassword } : {}), data: payload() });
      setResult(data);
