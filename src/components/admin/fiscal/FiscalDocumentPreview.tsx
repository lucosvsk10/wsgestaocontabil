  ...
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/75 p-3" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
    <div className="flex h-[96vh] w-full max-w-7xl flex-col overflow-hidden rounded-2xl border bg-background shadow-2xl">
      ...
      <div className="min-h-0 flex-1 overflow-auto bg-muted/20 p-3">
        {loading?<div className="flex h-full items-center justify-center gap-3 text-sm text-muted-foreground"><Loader2 className="h-5 w-5 animate-spin"/>Gerando {pdfLabel} para visualização...</div>:pdfUrl?<object data={`${pdfUrl}#toolbar=1&navpanes=0&view=FitH`} type="application/pdf" aria-label={title} className="h-full min-h-[70vh] w-full rounded-xl bg-white shadow-xl"><div className="flex h-full items-center justify-center p-8 text-center text-sm text-muted-foreground">O navegador não conseguiu exibir o PDF embutido. Use o botão “Baixar {pdfLabel}” para abrir o documento.</div></object>:<div className="mx-auto max-w-3xl rounded-2xl border bg-background p-6">
