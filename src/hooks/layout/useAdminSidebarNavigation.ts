...
import { LayoutDashboard, FileText, PieChart, Calculator, Settings, Wrench, HardDrive, Megaphone, Calendar, Images, FileStack, LucideIcon, Building2, ReceiptText, Send, Clock3 } from "lucide-react";
...
export const useAdminSidebarNavigation=()=>{
 ...
 const sidebarSections:SidebarSection[]=[
  ...
  {title:'Departamento Pessoal',items:[{icon:Clock3,label:'Calculadora de Horas',active:active('/admin/departamento-pessoal/horas'),to:'/admin/departamento-pessoal/horas'}]},
  {title:'Fiscal',items:[{icon:Send,label:'Emissor Fiscal',active:false,to:'/app?source=admin'},{icon:ReceiptText,label:'Extrator Fiscal',active:false,to:'/extrator?source=admin'}]},
  {title:'Clientes',items:[{icon:Building2,label:'Clientes',active:active('/admin/clientes')||active('/admin/assinantes')||active('/admin/fiscal/empresas'),to:'/admin/clientes'},{icon:FileText,label:'Documentos dos clientes',active:active('/admin/documentos')||active('/admin/user-documents'),to:'/admin/documentos'},{icon:HardDrive,label:'Armazenamento',active:active('/admin/storage'),to:'/admin/storage'},{icon:Megaphone,label:'Anúncios',active:active('/admin/announcements'),to:'/admin/announcements'},{icon:PieChart,label:'Enquetes',active:active('/admin/polls'),to:'/admin/polls'}]},
